...
CONFIG = {
    ...
    'MAX_WORKERS': 4,  # Reduced from 12 to prevent connection exhaustion
...
class IncrementalScraper:
    ...
    def process_single_car(self, listing: Dict, images_dir: str) -> Optional[Dict]:
        ...
        try:
            ...
            self.logger.error(f"Exception in process_single_car for {listing.get('uri', 'unknown')}: {type(e).__name__}: {e}")
            import traceback
    ...
    def run(self, max_new_listings: int = None) -> Dict:
        ...
        try:
            ...
            self.logger.info("Creating ThreadPoolExecutor...")
            with ThreadPoolExecutor(max_workers=CONFIG['MAX_WORKERS']) as executor:
                self.logger.info(f"ThreadPoolExecutor created with {CONFIG['MAX_WORKERS']} workers")
                
                # Submit all tasks
                future_to_listing = {}
                for i, listing in enumerate(new_listings):
                    try:
                        future = executor.submit(self.process_single_car, listing, images_dir)
                        future_to_listing[future] = listing
                        if (i + 1) % 100 == 0:
                            self.logger.debug(f"Submitted {i + 1}/{len(new_listings)} tasks")
                    except Exception as e:
                        self.logger.error(f"Failed to submit task {i}: {e}")
                
                self.logger.info(f"All {len(future_to_listing)} tasks submitted, waiting for results...")
                ...
                for future in as_completed(future_to_listing):
                    ...
                    try:
                        car_data = future.result(timeout=60)  # 60s timeout per car
                        if car_data:
                        ...
                        self.logger.error(f"Error processing {listing.get('uri', 'unknown')}: {type(e).__name__}: {e}")
                        self.stats.increment('error_count')
                        
                self.logger.info(f"Phase 2 completed: {len(processed_cars)} cars processed successfully")
                
        except Exception as e:
            self.logger.error(f"CRITICAL: ThreadPoolExecutor failed: {type(e).__name__}: {e}")
            import traceback
